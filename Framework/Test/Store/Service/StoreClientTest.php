...
class StoreClientTest extends TestCase
{
    ...
    public function testItReturnsUserInfo(): void
    {
        $userInfo = [
            'name' => 'John Doe',
            'email' => 'john.doe@snapadmin.net',
            'avatarUrl' => 'https://avatar.snapadmin.net/john-doe.png',
        ];
    ...
    public function testMissingConnectionBecauseYouAreInGermanCellularInternet(): void
    {
        $this->getStoreRequestHandler()->append(new ConnectException(
            'cURL error 7: Failed to connect to api.snapadmin.net port 443 after 4102 ms: Network is unreachable (see https://curl.haxx.se/libcurl/c/libcurl-errors.html) for https://api.snapadmin.net/swplatform/pluginupdates?shopwareVersion=6.4.12.0&language=de-DE&domain=',
